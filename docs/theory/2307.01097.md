...
$$\mathbf{m} = \sum_{l} \sum_{t_{*}^{l} \in \mathcal{N}(\mathbf{t}^{l})} \operatorname{SoftMax} \left( \left[ \mathbf{Q} \bar{\mathbf{F}}(\mathbf{s}) \right] \cdot \left[ \mathbf{K} \bar{\mathbf{F}}(t_{*}^{l}) \right] \right) \mathbf{V} \bar{\mathbf{F}}^{l}(t_{*}^{l}), \tag{2}$$
...
- [17] Jonathan Ho and Tim Salimans. Classier-free diffusion guidance. arXiv preprint arXiv:2207.12598, 2022.
...
- [20] Lukas Höllein, Ang Cao, Andrew Owens, Justin Johnson, and Matthias Nießner. Text2room: Extracting textured 3d meshes from 2d text-to-image models. arXiv preprint arXiv:2303.11989, 2023.
