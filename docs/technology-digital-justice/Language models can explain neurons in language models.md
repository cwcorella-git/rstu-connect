...
[Layer 0 neuron 816:](https://openaipublic.blob.core.windows.net/neuron-explainer/neuron-viewer/index.html#/layers/0/neurons/816) language related to Marvel comics, movies, and characters, as well as other superhero-themed content
...
- [Iterating on explanations](about:reader?url=https%3A%2F%2Fopenaipublic.blob.core.windows.net%2Fneuron-explainer%2Fpaper%2Findex.html#sec-revisions). We asked GPT-4 to come up with possible counterexamples, and to then revise explanations in light of their activations.
- [Using more capable models to give explanations](about:reader?url=https%3A%2F%2Fopenaipublic.blob.core.windows.net%2Fneuron-explainer%2Fpaper%2Findex.html#sec-explainer-scaling). The average score goes up as the explainer model's capability increases, with GPT-4 achieving the highest scores.
- [Using more capable models to simulate activations conditional on](about:reader?url=https%3A%2F%2Fopenaipublic.blob.core.windows.net%2Fneuron-explainer%2Fpaper%2Findex.html#sec-simulator-scaling) [an explanation](about:reader?url=https%3A%2F%2Fopenaipublic.blob.core.windows.net%2Fneuron-explainer%2Fpaper%2Findex.html#sec-simulator-scaling). Both the average score and the agreement between scores and human comparisons go up as the simulator model's capability increases, with GPT-4 achieving the highest scores.
...
- [Changing the architecture of the explained model](about:reader?url=https%3A%2F%2Fopenaipublic.blob.core.windows.net%2Fneuron-explainer%2Fpaper%2Findex.html#sec-subject-activation). We achieved higher explanation scores on models trained with different activation functions.
- [Searching for more explainable directions](about:reader?url=https%3A%2F%2Fopenaipublic.blob.core.windows.net%2Fneuron-explainer%2Fpaper%2Findex.html#sec-direction-finding). Our preliminary investigation into direct optimization of scores shows that we can find some well-explained linear combinations of neurons.
...
We are open sourcing our dataset of explanations for all neurons in GPT-2 XL, [code for explanation and scoring](https://github.com/openai/automated-interpretability/tree/main/neuron-explainer) to encourage further research in both producing better explanations. We are also releasing a [neuron viewer](https://openaipublic.blob.core.windows.net/neuron-explainer/neuron-viewer/index.html) using the dataset. Although most wellexplained neurons are not very interesting, we found [many](about:reader?url=https%3A%2F%2Fopenaipublic.blob.core.windows.net%2Fneuron-explainer%2Fpaper%2Findex.html#sec-interesting-neurons)

[interesting neurons](about:reader?url=https%3A%2F%2Fopenaipublic.blob.core.windows.net%2Fneuron-explainer%2Fpaper%2Findex.html#sec-interesting-neurons) that GPT-4 didn't understand. We hope this lets others more easily [build on top of our work.](about:reader?url=https%3A%2F%2Fopenaipublic.blob.core.windows.net%2Fneuron-explainer%2Fpaper%2Findex.html#sec-future-work) With better explanations and tools in the future, we may be able to rapidly uncover interesting qualitative understanding of model computations.
...
For both generating and simulating explanations, we take text excerpts from the training split of the subject model's pre-training dataset (e.g. Web Text, for GPT-2 models). We choose random 64 token contiguous subsequences of the documents as our text excerpts. Note that since GPT-2 models use byte-pair encoders, sometimes our texts have mid-character breaks. See [here for more](about:reader?url=https%3A%2F%2Fopenaipublic.blob.core.windows.net%2Fneuron-explainer%2Fpaper%2Findex.html#sec-limitation-tokenization) [discussion.](about:reader?url=https%3A%2F%2Fopenaipublic.blob.core.windows.net%2Fneuron-explainer%2Fpaper%2Findex.html#sec-limitation-tokenization), formatted identically to how the models were trained, with the exception that we never cross document boundaries. GPT-2 was trained to sometimes see multiple documents, separated by a special end of text token. In our work, we ensure all 64 tokens are within the same document.
...
- Given polysemanticity, neurons [could need extremely](about:reader?url=https%3A%2F%2Fopenaipublic.blob.core.windows.net%2Fneuron-explainer%2Fpaper%2Findex.html#sec-limitation-unexplainable) [long/disjunctive explanations.](about:reader?url=https%3A%2F%2Fopenaipublic.blob.core.windows.net%2Fneuron-explainer%2Fpaper%2Findex.html#sec-limitation-unexplainable) Even a neuron that corresponds to one feature could have substantial interference from other features. Explaining only the behavior at the extremes seemed like a reasonable way to get cleaner features, which may be more analogous to what we hope to be explaining in the long run.
- We wanted to be able to score on adversarially chosen text excerpts, e.g. for our [explanation revisions.](about:reader?url=https%3A%2F%2Fopenaipublic.blob.core.windows.net%2Fneuron-explainer%2Fpaper%2Findex.html#sec-revisions) It was more challenging to do this while maintaining the uniform-over-pretraining interpretation. In the long term, we want to move toward something debate-like where the score is more like a minimax than an expectation. That is, we imagine one model coming up with an explanation and another model coming up with counterexamples. Scoring would take the whole transcript into consideration, and thus measure how robust the hypothesis is. Furthermore, a perfect correlation (or explained variance) of 1 still has a similar interpretation.
...
In most places, we calculate this using 1.96 times the standard error of the mean (SEM), or a strictly more conservative statistic. If needed we estimate via bootstrap resampling methods. Overall, for GPT-2, we find an average score of 0.151 using topand-random scoring, and 0.037 for random-only scoring. Scores generally decrease when going to later layers.
...
We ran this algorithm on GPT-2 small's layer 10 (the penultimate

MLP layer), which has 3072 neurons. For each 3072-dimensional direction, we ran 10 rounds of coordinate ascent. For the gradient ascent we use Adam with a learning rate of 1e-2, \beta\_1 of 0.9, and \beta\_2 of 0.999. We also rescale \theta to be unit norm each iteration.
...
, IA (52732) Today Rain early...then remaining cloudy with showers in the afternoon. Thunder possible. High 66F. Winds light and variable. Chance of rain 80%.. Tonight Thunderstorms likely. Low

## around 60F. Winds SSE at 5 to 10 mph
...
Throughout the project we found many interesting neurons. GPT-4 was able to find explanations for non-trivial neurons that we thought were reasonable upon inspection, such as a ["simile"](https://openaipublic.blob.core.windows.net/neuron-explainer/neuron-viewer/index.html#/layers/25/neurons/2602) [neuron,](https://openaipublic.blob.core.windows.net/neuron-explainer/neuron-viewer/index.html#/layers/25/neurons/2602) [a neuron for phrases related to certainty and con](https://openaipublic.blob.core.windows.net/neuron-explainer/neuron-viewer/index.html#/layers/25/neurons/4870)fidence, and [a neuron for things done correctly.](https://openaipublic.blob.core.windows.net/neuron-explainer/neuron-viewer/index.html#/layers/14/neurons/417)

One successful strategy for finding interesting neurons was looking for those which were poorly explained by their token-space explanations, compared with their activation-based explanations. This led us to concurrently discover context neurons which activate densely in certain contexts We called these "vibe" neurons. and many neurons which activated on specific words at the beginning of documents.

Another related strategy that does not rely on explanation quality was to look for context-sensitive neurons that activate differently when the context is truncated. This led us to discover [a pattern](https://openaipublic.blob.core.windows.net/neuron-explainer/neuron-viewer/index.html#/layers/16/neurons/518) [break neuron](https://openaipublic.blob.core.windows.net/neuron-explainer/neuron-viewer/index.html#/layers/16/neurons/518) which activates for tokens that break an established pattern in an ongoing list (shown below on some select sentences) and [a post-typo neuron](https://openaipublic.blob.core.windows.net/neuron-explainer/neuron-viewer/index.html#/layers/18/neurons/3481) which activates often following strange or truncated words. Our explanation model is generally unable to get the correct explanation on interesting context-sensitive neurons.
...
We found some interesting examples of neurons that respond to specific kinds of repetition. We found a neuron that [activations for](https://openaipublic.blob.core.windows.net/neuron-explainer/neuron-viewer/index.html#/layers/4/neurons/4342) [repeated occurrences of tokens,](https://openaipublic.blob.core.windows.net/neuron-explainer/neuron-viewer/index.html#/layers/4/neurons/4342) with stronger activations depending on the number of occurrences. An interesting example of a polysemantic neuron is a neuron that fi[res for both the phrase](https://openaipublic.blob.core.windows.net/neuron-explainer/neuron-viewer/index.html#/layers/35/neurons/29) ["over and over again" and "things repeated right before a non](https://openaipublic.blob.core.windows.net/neuron-explainer/neuron-viewer/index.html#/layers/35/neurons/29)[repeated number",](https://openaipublic.blob.core.windows.net/neuron-explainer/neuron-viewer/index.html#/layers/35/neurons/29) possibly because "over and over again" itself includes repetition. We also found [two](https://openaipublic.blob.core.windows.net/neuron-explainer/neuron-viewer/index.html#/layers/19/neurons/1763) [neurons](https://openaipublic.blob.core.windows.net/neuron-explainer/neuron-viewer/index.html#/layers/20/neurons/3164) that seem mostly to respond to a second mention of a surname when combined with a different first name. It is possible that these neurons are responding to induction heads.
...
For more interesting neurons, see our [neuron visualization](https://openaipublic.blob.core.windows.net/neuron-explainer/neuron-viewer/index.html) [website.](https://openaipublic.blob.core.windows.net/neuron-explainer/neuron-viewer/index.html)

#### Explaining constructed puzzles

When thinking about how to qualitatively understand our explanation methodology, we often ran into two problems. First, we do not have any ground truth for the explanations or scores. Even human-written explanations could be incorrect, or at the very least fail to completely explain the behavior. Furthermore, it is often difficult to tell whether a better explanation exists. Second, we have no control over the complexity or types of patterns neurons encode, and [no guarantee that any simple explanation exists.](about:reader?url=https%3A%2F%2Fopenaipublic.blob.core.windows.net%2Fneuron-explainer%2Fpaper%2Findex.html#sec-limitation-unexplainable)
...
We are releasing [code for trying out the neuron puzzles we](https://github.com/openai/automated-interpretability/blob/main/neuron-explainer/demos/explain_puzzles.ipynb) [constructed.](https://github.com/openai/automated-interpretability/blob/main/neuron-explainer/demos/explain_puzzles.ipynb)
...
On the other hand, this is perhaps favorable compared to pretraining itself. If pre-training scales data approximately linearly with parameters, then it uses compute O(n^2).
