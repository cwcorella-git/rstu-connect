# Library Audit Tracking

Started: 2026-01-17
Current document count: 2,443

## Category Overview

| Category | Count | Status | Notes |
|----------|-------|--------|-------|
| contemporary-analysis | ~696 | **in progress** | Removed 204+ docs (lectures, video game wikis, general Wikipedia, screenplays, self-help, AI/tech, psychology papers) |
| labor | 449 | pending | |
| theory | 252 | pending | |
| environmental-justice | 179 | pending | |
| technology-digital-justice | 174 | pending | |
| arts-culture-music | 122 | pending | |
| abolition | 106 | pending | |
| housing | 103 | pending | Core content for tenant organizing |
| feminist-theory | 99 | pending | |
| organizing | 67 | pending | Core content for tenant organizing |
| anti-war-peace | 51 | pending | |
| international-solidarity | 41 | pending | |
| economic-alternatives | 36 | **audited** | Good Graeber content |
| youth-student-organizing | 30 | **audited** | Removed 3 irrelevant |
| legislation | 15 | **audited** | Removed 1 irrelevant (Minnesota budget) |
| food-justice | 9 | **audited** | Removed 3 irrelevant architecture articles |
| indigenous-solidarity | 7 | **audited** | Some miscategorized - needs recategorization |
| misc | 2 | **audited** | Removed 2 irrelevant federal land docs |
| disability-justice | 2 | **audited** | 1 prison brutality report, 1 civil resistance guide |
| data-requests | 2 | **internal** | FOIA request templates - repo only |
| governance | 1 | **internal** | App governance docs - repo only |

---

## Internal/Repo-Only Documents

Documents that should be accessible to Claude for reference but not displayed in the public library.

| File | Reason |
|------|--------|
| data-requests/washoe-code-enforcement-request.md | FOIA template for code enforcement data |
| data-requests/washoe-eviction-data-request.md | FOIA template for eviction data |
| governance/App_Governance_System.md | Internal app governance documentation with code refs |
| misc/proposed-bylaws-to-review-at-2-21-meeting.pdf.md | RSTU internal bylaws draft |
| notes/* | Internal notes and tracking |

---

## Documents Removed (24 total)

| Category | File | Reason |
|----------|------|--------|
| misc | Federal land policy in Nevada - Ballotpedia.md | Federal land policy, not tenant-related |
| misc | Local government perspectives of federal land management in Nevada - ScienceDirect.md | Academic paper on federal land |
| food-justice | noa tops library concept in milan with organic roof ring + blossoming park.md | Architecture article about library design |
| food-justice | vo trong nghia's 'urban farm office' returns tropical nature to the city.md | Architecture article about office design |
| food-justice | Catalhoyuk Urban Life in Neolithic Anatolia.md | Archaeology article about ancient Turkey |
| legislation | Budget Negotiations Continue as Legislature Prepares for Special Session - League of Minnesota Cities.md | Minnesota state budget, not Nevada |
| youth-student-organizing | Comparing Patricians and Plebeians.md | Ancient Roman history |
| youth-student-organizing | Ken Robinson (educationalist) - Wikipedia.md | Wikipedia biography of education theorist |
| youth-student-organizing | Himeyuri students - Wikipedia.md | WWII history about Japanese student nurses |
| contemporary-analysis | 2022_TaxReturn.md | **PERSONAL TAX RETURN DATA** - should never have been public |
| contemporary-analysis | Albert Ellis - Independent Online Edition _ Obituaries.md | Obituary of psychologist |
| contemporary-analysis | alien-1978-10-04.md | Movie screenplay (Alien) |
| contemporary-analysis | Bone-rank system - Wikipedia.md | Korean historical caste system |
| contemporary-analysis | Cognitive dissonance - Wikipedia.md | Psychology Wikipedia article |
| contemporary-analysis | Contact_1997_Screenplay.md | Movie screenplay (Contact) |
| contemporary-analysis | Albert Cashier - Wikipedia.md | Trans Civil War soldier biography |
| contemporary-analysis | Confederate States of America - Wikipedia.md | CSA Wikipedia article |
| contemporary-analysis | How to be a friend an ancient guide to true friendship.md | Corrupted/garbled content |
| contemporary-analysis | How to be a friend an ancient guide to tru - Marcus Tullius Cicero.md | Ancient Roman self-help |
| contemporary-analysis | How to Grow Old Ancient Wisdom for the Sec - Marcus Tullius Cicero.md | Ancient Roman self-help |
| contemporary-analysis | How to Grow Old Ancient Wisdom for the Second Half of Life.md | Ancient Roman self-help |
| contemporary-analysis | Cicero-Country.indb - How to Run a Country...md | Ancient Roman political philosophy |
| contemporary-analysis | COVID-19 pandemic - Wikipedia.md | General pandemic Wikipedia |
| contemporary-analysis | Why the world needs viruses to function - BBC Future.md | General science article |
| arts-culture-music | Artificial Intelligence and Digital Immortality_ Will AI Raise the Dead_.md | Pop culture/tech speculation |

---

## Titles to Fix

| Category | File | Current Title | Suggested Title |
|----------|------|---------------|-----------------|
| disability-justice | Alabama Correctional Officers...md | Truncated | "Prison Guards Use Deadly Force in Mental Health Crisis - Free Alabama Movement Report" |
| contemporary-analysis | edit (1).md | "Soil Solidarity" | "Soil Solidarity - Housing Solutions Letter to Faith Leaders" |
| contemporary-analysis | 1, 2001 -- William Morrow...Anna's Archive.md | "Bell hooks" | "All About Love - bell hooks" |
| contemporary-analysis | Large print, 2023 -- Random...Anna's Archive.md | "BROOKS" | "How to Know a Person - David Brooks" |
| contemporary-analysis | HarperCollins -- 9780008354657...Anna's Archive.md | "WILL STORR" | "The Status Game - Will Storr" |

---

## Missing Metadata

### Missing Authors
| Category | File | Suggested Author |
|----------|------|------------------|

### Missing Dates
| Category | File | Suggested Date |
|----------|------|----------------|

### Missing/Wrong Tags
| Category | File | Suggested Tags |
|----------|------|----------------|
| governance | App_Governance_System.md | Has "wikipedia" tag - should be "rstu", "governance", "internal" |

---

## Documents to Recategorize

| Current Category | File | Suggested Category | Reason |
|-----------------|------|-------------------|--------|
| indigenous-solidarity | Obsolete communism The left wing alternative - Daniel and Gabriel Cohn-Bendit.md | theory | May 1968 France, not indigenous |
| legislation | Holding the line supporters picket Canada Post after back-to-work legislation.md | labor | IWW organizing, solidarity pickets |
| food-justice | blueprints vol. vii rice farming updates.md | organizing or economic-alternatives | Food sovereignty in Sierra Leone, good organizing content |

---

## Category Notes

### disability-justice (2 files) ✓
- **Alabama Correctional Officers...** - Prison brutality, mental health crisis. ~600 words. Related to abolition. Consider moving to abolition category.
- **Chronicling-Civil-Resistance.md** - EXCELLENT comprehensive guide for journalists on civil resistance (~5000 words). Very valuable organizing content. KEEP.

### misc (2 files remaining) ✓
- **proposed-bylaws-to-review-at-2-21-meeting.pdf.md** - RSTU bylaws draft. Mark as internal.

### data-requests (2 files) ✓
Both are FOIA request templates. Internal use only.

### governance (1 file) ✓
App governance documentation with code references. Internal use only.

### food-justice (9 files remaining) ✓
Removed 3 off-topic architecture articles. Remaining content includes:
- Bookchin on radical agriculture
- Seed sovereignty / anti-Monsanto content
- Food sovereignty organizing content

### legislation (15 files remaining) ✓
Removed Minnesota budget article. Remaining is mostly Nevada state legislation.

### youth-student-organizing (30 files remaining) ✓
Removed 3 Wikipedia/history articles. Remaining includes:
- IWW and student worker organizing
- UC graduate student strikes
- Student movement history (Chile, France 1968)

### indigenous-solidarity (7 files) ✓
Some miscategorized - includes Dakota Pipeline content (relevant) but also May 1968 France content (should be in theory).

### economic-alternatives (36 files) ✓
Good collection - lots of David Graeber on debt, class struggle theory, Pannekoek.

---

## Short Documents (<200 words)

Documents that may be stubs or need expansion.

| Category | File | Words | Action |
|----------|------|-------|--------|

---

## Audit Log

### 2026-01-17
- Created tracking file
- Initial category counts established
- **Audited small categories:** governance, data-requests, misc, disability-justice
- Removed 2 irrelevant documents (federal land policy articles)
- Identified 5 internal/repo-only documents
- **Audited medium categories:** food-justice, legislation, youth-student-organizing, indigenous-solidarity, economic-alternatives
- Removed 7 more irrelevant documents
- Identified 3 documents needing recategorization
- **Started auditing large categories:** contemporary-analysis, arts-culture-music
- Found and removed CRITICAL: personal tax return with SSN exposed
- Removed movie screenplays (Alien, Contact)
- Removed ancient Roman self-help books (Cicero)
- Removed general Wikipedia articles (COVID, Confederate States, etc.)
- **Total removed: 24 documents**
- **Current count: 2,649**

### 2026-01-19 (Session 2)
- Continued contemporary-analysis audit
- **Removed 31 "Episode" files** - Vervaeke's "Awakening from the Meaning Crisis" lecture series (cognitive science, not organizing)
- Removed 8 self-help/tutorial files (Cicero election guides, juniper plant care, 3D modeling, networking books)
- Removed Gandhi health guide, James Joyce literary criticism, Uncle Tom's Cabin
- Removed Stanford Encyclopedia philosophy articles (Aristotle, Epictetus, Plato)
- Removed Joseph Campbell "Power of Myth", AI benchmark guide, Albert Ellis obituary
- **Session 2 removed: 48 documents**
- **Total removed: 72 documents**
- **Current count: 2,601**
- Next: continue contemporary-analysis audit, then other large categories

### 2026-01-20 (Session 3)
- Continued contemporary-analysis audit
- **Removed 5 video game wiki pages** - Outriders Wiki, Wookieepedia (Star Wars) content
- **Removed 3 technical tutorials** - SPH physics simulation, Olympics media guide, creative writing guide
- **Removed 25+ general Wikipedia articles** - psychology (Facial expression, Empathy, Deference, Pronoia), philosophy (Cosmopolitanism, Teleology, Aristotle Metaphysics), general history (Athenian democracy, Hypatia, Jesse Owens, Robert E. Lee, Mexican Cession, etc.)
- **Removed 3 misplaced books** - Colleen Hoover romance novel, H.G. Wells sci-fi, sports coaching book
- **Removed stray files** - Game developer pitch doc, Unity plugin documentation, video game manual (Dead Space), rowing federation manual
- **Removed duplicate files** - "Information wants to be free" duplicates (1) and (2)
- **Identified files needing filename fixes** - "edit (1).md" is actually Soil Solidarity housing letter, Anna's Archive books need proper titles
- **Session 3 removed: 42 documents**
- **Total removed: 114 documents**
- **Current count: 2,559**
- Next: continue contemporary-analysis audit (still ~812 files), then labor and theory categories

### 2026-01-20 (Session 4 - continued)
- Continued contemporary-analysis audit
- **Removed 5 more Wikipedia articles** - Proclamation 80, Battle Cry of Freedom, LBJ, Bretton Woods Conference/system
- **Removed 4 entertainment files** - Becky Chambers bio, Galactic Republic (Star Wars), Shawshank Redemption screenplay, Patricia Wrede writing advice
- **Removed 8 screenplays/creative writing** - Citizen Kane, Parasite, Eternal Sunshine, No Country for Old Men, plus writing guides
- **Removed technical/academic papers** - AI research (DreamBooth, Transformer Memory), Korean archaeology, military desalination, robotics
- **Removed miscellaneous** - Game design templates (Silent Hill, Chris Taylor), thesis presentations (architecture), religious texts (Gandhi prayer, Word of Buddha), NPR religion article, Victorian design book
- **Session 4 removed: 32 documents**
- **Total removed: 146 documents**
- **Current count: 2,527**
- Next: continue contemporary-analysis (780 files), then move to labor category

### 2026-01-20 (Session 5 - continued)
- Continued contemporary-analysis audit
- **Removed self-help/pop psychology** - Atomic Habits, Road Less Traveled, Make It Stick, Parasitic Processing (overthinking), Corruptible (power psychology)
- **Removed AI/tech books** - Benevolent By Design, Natural Language Cognitive Architecture, Symphony of Thought (all AI/AGI)
- **Removed design/architecture** - Design of Everyday Things, Eyes of the Skin, chatbot design, Japanese beach house, Portugal hotel cabins, PC power supply guide
- **Removed science Wikipedia** - Kinesiology, Stellarator (nuclear fusion), Thermoreceptor (biology)
- **Removed ancient philosophy** - Pythagoras Wikipedia, Nicomachean Ethics (Aristotle)
- **Removed academic papers** - Sexual promiscuity research, cognitive neuroscience deception, expressive order (sociology)
- **Session 5 removed: 24 documents**
- **Total removed: 170 documents**
- **Current count: 2,503**
- Next: continue contemporary-analysis (756 files), then move to labor category

### 2026-01-20 (Session 6 - continued)
- Continued contemporary-analysis audit
- **Removed psychology/cognitive science papers** - Cognitive Dissonance (2 papers), Emotional Intelligence, Emotional Contagion, Cognitive Mechanisms Deception
- **Removed physics/engineering papers** - Bounce Maps rigid-body simulation, Nanodiamonds for energy, Collision detection
- **Removed AI/tech content** - LessWrong deceptive alignment, Steam recommender news, HCI meeting clustering, Nature brain/speech predictive coding, Symphony of Thought (AI book)
- **Removed screenplays/fiction** - Letters from Iwo Jima, N.K. Jemisin Emergency Skin
- **Removed pop poetry** - Rupi Kaur (Milk and Honey, Home Body)
- **Removed general Wikipedia** - Dharma, Ares, Plebeians, Kudzu (invasive plant)
- **Removed misc** - Japanese folk song analysis, Korean archaeology, Philosophy of "chmess", Claude AI conversation, MS Research hybrid work paper, Piaggio scooter manual, Paolo Soleri arcology, Aposimz manga, video game industry book
- **Session 6 removed: 60 documents**
- **Total removed: 230 documents**
- **Current count: 2,443**
- **Contemporary-analysis: 696 files remaining** (mostly high-quality organizing content now)
- **Additional removals:** FARGO, Jaws, Matrix, Sorry to Bother You screenplays; Patricia Wrede writing advice; Susan RoAne networking; US Army field manual; Derinkuyu archaeology (x2); AR glasses tech news (x2); Reeves masculinity book; battery tech news; Minnesota Wikipedia; Nevada species list; DTI medical imaging; CRISPR genetics paper
- Next: labor category (449 files), then theory (252 files)

