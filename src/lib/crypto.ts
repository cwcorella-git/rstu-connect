...
/**
 * Clear the cached key (for logout/cleanup)
 */
...
/**
 * Rotate the encryption key (re-encrypts all sensitive data)
 * Call this periodically or after suspected compromise
 */
export async function rotateKey(sensitiveKeys: string[]): Promise<void> {
  if (typeof window === 'undefined') return

  // Decrypt all sensitive data with old key
  const decryptedData: Record<string, string> = {}
  for (const key of sensitiveKeys) {
    const value = await getEncrypted(key)
    if (value) {
      decryptedData[key] = value
    }
  }

  // Clear cached key and stored key
  cachedKey = null
