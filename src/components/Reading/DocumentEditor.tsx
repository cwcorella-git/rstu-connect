...
export function DocumentEditor({ document, initialContent, onClose, onSave }: DocumentEditorProps) {
  ...
  const handleRevert = async () => {
    if (confirm(t('reading.revertConfirm') || 'Are you sure you want to discard all edits and restore the original document?')) {
      ...
      <div className="bg-white w-full max-w-6xl h-[90vh] rounded-lg shadow-xl flex flex-col">
        ...
        <div className="p-4 border-b border-gray-200 flex items-center justify-between">
          <div className="flex-1">
            <h2 className="text-lg font-bold text-gray-900">{t('reading.editDocument') || 'Edit Document'}</h2>
            <p className="text-xs text-gray-500 mt-1">
              {t('reading.changesLocalSaved') || 'Changes are saved locally. Export edits to create a patch file.'}
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-hidden flex flex-col p-4 space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              {t('reading.documentTitle') || 'Document Title'}
            </label>
            <input
              type="text"
              value={title}
              ...
              placeholder={t('reading.documentTitlePlaceholder') || "Document title"}
            />
          </div>
          ...
          <div className="flex-1 flex flex-col">
            <label className="block text-sm font-medium text-gray-700 mb-1">
              {t('reading.markdownContent') || 'Markdown Content'}
            </label>
            <textarea
              value={content}
          ...
          {hasChanges && (
            <div className="text-xs text-amber-600 bg-amber-50 border border-amber-200 rounded px-3 py-2">
              {t('reading.unsavedChanges') || 'You have unsaved changes'}
            </div>
          )}
        </div>
        ...
        <div className="p-4 border-t border-gray-200 flex items-center justify-between">
          ...
          >
            {t('reading.revertOriginal') || 'Revert to Original'}
          </button>
