...
import { checkAdminAuth, logoutAdmin } from '@/lib/storage/adminStorage'
...
// Tab icon mapping - returns JSX for each tab's icon
function getTabIcon(tab: Tab): JSX.Element {
...
export function HamburgerMenu({ isOpen, onClose }: HamburgerMenuProps) {
  ...
  const { isAuthenticated, canAccessToolsTab, canAccessOrganizeTab, profile, refreshAuth, isAdminAuthenticated } = useAuth()
  const { t } = useLanguage()
  ...
  // Get dynamic navigation order and filter based on permissions
  const navOrder = getEffectiveNavOrder().filter(tab => {
    if (tab === 'home' && !canAccessOrganizeTab) return false
    if (tab === 'tools' && !canAccessToolsTab) return false
    if (tab === 'profile' || tab === 'governance') return false // Profile and governance in separate sections
    return true
  })
        ...
        <div className="py-4 flex flex-col h-[calc(100%-60px)]">
          ...
          <div className="border-t border-gray-200 mt-4 pt-4">
            ...
            >
              ...
              {isAuthenticated ? getEffectiveNavLabel('profile', t) : t('nav.loginCreate')}
            </button>

            {/* Admin Logout - only visible to admins */}
            {isAdminAuthenticated && (
              <button
                onClick={handleAdminLogout}
                className="w-full px-4 py-3 text-left flex items-center gap-3 text-gray-500 hover:bg-gray-50"
              >
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
                </svg>
                {t('nav.adminLogout') || 'Admin Logout'}
              </button>
            )}
          </div>
        </div>
