...
import { getLinkedGroups, getGroupForApn, generateBlocName, type LinkedPropertyGroup } from '@/lib/linkedPropertiesStorage'
...
export function BlocJoinProposal({
  ...
  // Check if current building is already in a bloc
  ...
  // Get addresses for a bloc
  const getBlocAddresses = (group: LinkedPropertyGroup): string[] => {
    return group.apns
      .map(apn => buildings.find(b => b.apn === apn)?.address)
      .filter((addr): addr is string => !!addr)
  }

  // Get display name for bloc
  const getBlocDisplayName = (group: LinkedPropertyGroup): string => {
    if (group.name) return group.name
    const addresses = getBlocAddresses(group)
  ...
  const handleSubmit = () => {
    ...
    // Create join-bloc proposal
    // This requires approval from both this building AND the target bloc
    const proposal = createProposal('join-bloc', currentBuilding.chatSlug, {
      targetGroupId: selectedBloc.id,
      reason: reason.trim() || `Request to join ${getBlocDisplayName(selectedBloc)}`,
    })

    if (proposal) {
      const message = `[GOV:join-bloc:${currentBuilding.chatSlug}:${selectedBloc.id}] ${reason || `Proposing to join "${getBlocDisplayName(selectedBloc)}".`}`
      onSubmit(message)
    }
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div className="relative bg-white rounded-lg shadow-xl w-full max-w-md mx-4 p-6">
          ...
          <p className="text-sm text-gray-600 mb-4">
            ...
            You would need to leave your current bloc before joining another.
          </p>
          <button
            onClick={onClose}
      ...
      <div className="relative bg-white rounded-lg shadow-xl w-full max-w-lg mx-4 max-h-[85vh] flex flex-col">
        ...
        <div className="flex items-center justify-between p-4 border-b border-gray-200 flex-shrink-0">
          <div>
            ...
            <p className="text-xs text-gray-500">Select an existing bloc to request membership</p>
          </div>
          ...
          <div className="mb-4">
            ...
            {availableBlocs.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-xs text-gray-400 mt-1">Consider forming a new bloc instead!</p>
              </div>
            ) : (
              <div className="space-y-2">
                {availableBlocs.map(bloc => {
                  const isSelected = selectedBloc?.id === bloc.id
                  const addresses = getBlocAddresses(bloc)
                  const displayName = getBlocDisplayName(bloc)
