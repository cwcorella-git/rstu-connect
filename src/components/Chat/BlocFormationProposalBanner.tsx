...
interface BlocFormationProposalBannerProps {
  buildingApn: string
  chatSlug: string
  onProposalStatusChange?: () => void
}

export function BlocFormationProposalBanner({
  buildingApn,
  chatSlug,
  onProposalStatusChange
}: BlocFormationProposalBannerProps) {
  const [proposals, setProposals] = useState<GovernanceProposal[]>([])
  const [userVotes, setUserVotes] = useState<Record<string, 'up' | 'down' | null>>({})

  useEffect(() => {
    // Get all form-bloc proposals where this building is involved
              ...
              <div className="flex-1 min-w-0">
                ...
                {hasReachedThreshold && (
                  <div className="mt-3 text-xs text-green-700 font-medium">
                    ✓ This building has approved the bloc formation. Waiting for other buildings to vote...
                  </div>
                )}
              </div>
            </div>
