...
export function LinkedPropertiesList({ buildings, onSelectBuilding }: LinkedPropertiesListProps) {
  ...
  const handleDelete = (id: string) => {
    if (confirm('Delete this linked group?')) {
  ...
  if (groups.length === 0) {
    return (
      <div className="p-4 text-center text-gray-500">
        ...
        <p className="text-sm">No linked property groups yet.</p>
        <p className="text-xs mt-1 text-gray-400">
          Ctrl+click properties on the map or list, then press L to link them.
        </p>
      </div>
    )
  }
              ...
              <>
                <div className="flex-1">
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    {group.apns.length} properties linked
                  </p>
                </div>
