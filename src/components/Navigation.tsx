...
export function Navigation() {
  ...
  // Real-time unread count from Socket.io (shows on Profile button when authenticated)
  ...
  return (
    <>
      ...
      <div className="md:hidden flex items-center gap-2">
        {/* Login/Profile button with unread badge */}
        <button
          onClick={() => setActiveTab('profile')}
          className={`relative text-sm px-2.5 py-1 rounded-md border transition-colors ${
            activeTab === 'profile'
        ...
        >
          {isAuthenticated ? t('nav.profile') : t('nav.login')}
          {isAuthenticated && unreadCount > 0 && (
            <span className="absolute -top-1.5 -right-1.5 bg-red-500 text-white text-xs w-4 h-4 rounded-full flex items-center justify-center border border-white">
              {unreadCount > 9 ? '9+' : unreadCount}
            </span>
          )}
        </button>
