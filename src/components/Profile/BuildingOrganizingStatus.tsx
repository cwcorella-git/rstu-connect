  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 p-4">
      <div className="flex items-center justify-between mb-3">
        <h3 className="font-medium text-gray-900">{t('buildingOrg.title')}</h3>
      ...
      <div className="space-y-2 mb-4">
        <div className="flex justify-between text-sm">
          <span className="text-gray-600">{t('buildingOrg.participation')}</span>
        ...
        <p className="text-xs text-gray-400">{t('buildingOrg.strikeReady')}</p>

        {/* Canvassing Coverage */}
        {canvassStats.total > 0 && (
          <>
            <div className="flex justify-between text-sm mt-3">
              <span className="text-gray-600">{t('buildingOrg.canvassing')}</span>
              <span className="font-medium">
            ...
            <div className="flex gap-3 text-xs text-gray-500">
              {canvassStats.interested > 0 && (
                <span className="text-green-600">{canvassStats.interested} {t('buildingOrg.interested')}</span>
              )}
              {canvassStats.activeMembers > 0 && (
                <span className="text-purple-600">{canvassStats.activeMembers} {t('buildingOrg.active')}</span>
              )}
              {canvassStats.followUp > 0 && (
                <span className="text-orange-600">{canvassStats.followUp} {t('buildingOrg.followUp')}</span>
              )}
            </div>
          </>
        )}
        ...
        <div className="flex justify-between text-sm mt-3">
          <span className="text-gray-600">{t('buildingOrg.activeIssues')}</span>
          <span className="font-medium">{activeIssues} {t('buildingOrg.underVote')}</span>
        </div>
        <div className="flex justify-between text-sm">
          <span className="text-gray-600">{t('buildingOrg.demands')}</span>
          <span className="font-medium">{demandsCount} {t('buildingOrg.approved')}</span>
        </div>
      </div>
